...
namespace TernarySearchTree
{
    ...
    /// <summary>
    /// Added support for duplicate keys.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public class TernaryTreeExtended<T>
    {
            ...
            Node Add(Node node, string key, T value, int charIndex)
            {
                ...
                else
                {
                    ...
                    {
                        Array.Resize<T>(ref node.value, node.value.Length + 1);
                        node.value[node.value.Length - 1] = value;
